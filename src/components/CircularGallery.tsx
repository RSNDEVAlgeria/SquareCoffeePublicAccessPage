...
class App {
  ...
  onClick(e: MouseEvent) {
    if (!this.onImageClick || !this.medias.length) return;
    ...
    const clickOffset = (clickX / viewWidth - 0.5) * this.viewport.width;
    const adjustedOffset = clickOffset + scrollOffset;
    
    const totalWidth = itemWidth * this.mediasImages.length;
    const normalizedOffset = ((adjustedOffset % totalWidth) + totalWidth) % totalWidth;
    
    const centerIndex = Math.round(normalizedOffset / itemWidth);
    const uniqueIndex = centerIndex % (this.mediasImages.length / 2);
    
    if (uniqueIndex >= 0 && uniqueIndex < this.mediasImages.length / 2) {
      this.onImageClick(uniqueIndex);
    }
  }
...
export default function CircularGallery({
  ...
  const uniqueItems = items && items.length > 0 ? items : [
    { image: 'https://picsum.photos/seed/1/800/600', text: 'Image 1' },
    { image: 'https://picsum.photos/seed/2/800/600', text: 'Image 2' },
    { image: 'https://picsum.photos/seed/3/800/600', text: 'Image 3' },
    { image: 'https://picsum.photos/seed/4/800/600', text: 'Image 4' },
  ];

  const handlePrev = useCallback(() => {
    setSelectedIndex(prev => prev !== null ? (prev - 1 + uniqueItems.length) % uniqueItems.length : null);
  }, [uniqueItems.length]);

  const handleNext = useCallback(() => {
    setSelectedIndex(prev => prev !== null ? (prev + 1) % uniqueItems.length : null);
  }, [uniqueItems.length]);
    ...
    <>
      ...
      {selectedIndex !== null && (
        ...
        >
          ...
          >
            <img
              src={uniqueItems[selectedIndex].image}
              alt=""
